...
/* Must track the fixed readings in drivers/fuel_gauge/bq41z50/emul_bq41z50.c. */
#define EXP_TEMPERATURE_DK    2980
#define EXP_VOLTAGE_UV        (14400 * 1000)
...
#define EXP_MFR_ACCESS        0x0001
#define EXP_CAP_ALARM         300
#define EXP_TIME_ALARM        10
#define EXP_BATTERY_MODE      0x0000
...
ZTEST_USER_F(bq41z50, test_get_props__returns_ok)
{
	...
	zassert_equal(vals[7].sbs_mfr_access_word, EXP_MFR_ACCESS);
...
/*
 * OperationStatus (0x54) cannot be read while the gauge is sealed, so a production pack is
 * allowed to reject this property instead of reporting the XCHG bit.
 */
ZTEST_USER_F(bq41z50, test_get_charge_cutoff)
{
	union fuel_gauge_prop_val val;
	int ret = fuel_gauge_get_prop(fixture->dev, FUEL_GAUGE_CHARGE_CUTOFF, &val);

#if CONFIG_EMUL
	zassert_ok(ret);
	zassert_false(val.cutoff);
#else
	zassert_true(ret == 0 || ret == -EIO, "unexpected status %d", ret);
#endif
}
...
ZTEST_USER_F(bq41z50, test_set_props__round_trip)
{
	static const struct {
		...
		{FUEL_GAUGE_SBS_REMAINING_CAPACITY_ALARM, 250},
		{FUEL_GAUGE_SBS_REMAINING_TIME_ALARM_MINS, 15},
		{FUEL_GAUGE_SBS_ATRATE, 100},
		{FUEL_GAUGE_SBS_MFR_ACCESS, 0x0002},
	};
	...
	for (size_t i = 0; i < ARRAY_SIZE(cases); i++) {
		...
		zassert_ok(fuel_gauge_get_prop(fixture->dev, cases[i].prop, &original),
			   "prop %u read failed", cases[i].prop);

		probe.sbs_mfr_access_word = cases[i].probe;
		zassert_ok(fuel_gauge_set_prop(fixture->dev, cases[i].prop, probe),
			   "prop %u write failed", cases[i].prop);

		zassert_ok(fuel_gauge_get_prop(fixture->dev, cases[i].prop, &readback));
		zassert_equal(readback.sbs_mfr_access_word, cases[i].probe,
			      "prop %u did not read back", cases[i].prop);

		zassert_ok(fuel_gauge_set_prop(fixture->dev, cases[i].prop, original));
	}
}
